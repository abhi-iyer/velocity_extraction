...
class PINV_Experiment():
    def __init__(self, experiment_name, config):
        ...
        output_dir = './pinv_checkpoints/' + self.experiment_name
    ...
    def train_epoch(self):
        ...
        for patch1, patch2, _ in self.train_loader:
            patch1 = patch1.to(self.device)
            patch2 = patch2.to(self.device)

            recons = self.model(patch1.squeeze(), patch2.squeeze())

            if isinstance(self.loss_fn, SSIM_Regular_Loss):
                loss = self.loss_fn(recons.unsqueeze(1), patch2)
            elif isinstance(self.loss_fn, SSIM_Contrastive_Loss):
                loss = self.loss_fn(recons.unsqueeze(1), patch1, patch2)
            elif isinstance(self.loss_fn, ContrastV):
                v_11 = self.model.get_v(patch1.squeeze(), patch1.squeeze())
                v_22 = self.model.get_v(patch2.squeeze(), patch2.squeeze())
                v_12 = self.model.get_v(patch1.squeeze(), patch2.squeeze())
                v_21 = self.model.get_v(patch2.squeeze(), patch1.squeeze())

                loss = self.loss_fn(recons.unsqueeze(1), patch2, v_11, v_22, v_12, v_21)
            else:
                loss = self.loss_fn(recons.unsqueeze(1), patch2)
    ...
    def test_epoch(self):
        ...
        with torch.no_grad():
            for patch1, patch2, _ in self.test_loader:
                patch1 = patch1.to(self.device)
                patch2 = patch2.to(self.device)
